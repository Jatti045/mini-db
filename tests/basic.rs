...
fn insert_adds_new_row() {
    ...
    db.insert(1, "Alice".into(), 20).expect("insert should succeed");
...
fn select_returns_all_inserted_rows() {
    let mut db = Database::new();
    ...
    db.insert(1, "name".to_string(), 20).expect("insert should succeed");
    db.insert(2, "name".to_string(), 30).expect("insert should succeed");
...
fn insert_rejects_duplicate_ids() {
    ...
    assert!(matches!(err, Err(DbError::DuplicateId(1))));
}

#[test]
// Tests parsing logic and ensures correct command for user input is returned
fn parse_valid_insert_returns_command() {
    let input = "insert 1 alice 30";

    let command = parser::parse_command(input);
    assert_eq!(command, Ok(parser::Command::Insert { id: 1, name: "alice".into(), age: 30 }));
}
